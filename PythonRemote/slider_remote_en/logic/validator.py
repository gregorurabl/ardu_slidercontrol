...
def validate_normal(speed_steps: int, distance_steps: int, ramp_steps: int) -> Optional[str]:
    ...
    from logic.calibration_manager import get_distance_long
    dist_max = get_distance_long()
    ...
    if not (-dist_max <= distance_steps <= dist_max) or distance_steps == 0:
        return f"Distance must be between -{dist_max} and {dist_max} steps (non-zero)."
