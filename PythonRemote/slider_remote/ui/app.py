# =============================================================================
# ui/app.py – Hauptfenster
# Baut das gesamte Layout auf: Verbindungsleiste, Tabs, Aktionsbuttons, Log.
# =============================================================================
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Serial Handler – on_receive-Callback schreibt in das Log-Feld
    ...
    def _build_connection_bar(self):
        ...
        ctk.CTkButton(frame, text="Aktualisieren", width=100,
                      command=self._refresh_ports).pack(side="left", padx=4)

        self.connect_btn = ctk.CTkButton(frame, text="Verbinden", width=110,
                                         command=self._toggle_connection)
        self.connect_btn.pack(side="left", padx=4)

        self.status_label = ctk.CTkLabel(frame, text="Getrennt", text_color="tomato")
    ...
    def _refresh_ports(self):
        ...
        if ports:
            ...
            self.port_dropdown.configure(values=["Kein Port gefunden"])
            self.port_var.set("Kein Port gefunden")

    def _toggle_connection(self):
        if self.serial.is_connected():
            self.serial.disconnect()
            self.connect_btn.configure(text="Verbinden")
            self.status_label.configure(text="Getrennt", text_color="tomato")
            self._log("Verbindung getrennt.")
        else:
            try:
                self.serial.connect(self.port_var.get())
                self.connect_btn.configure(text="Trennen")
                self.status_label.configure(text="Verbunden", text_color="lightgreen")
                self._log(f"Verbunden mit {self.port_var.get()} @ 115200 baud.")
    ...
    # -------------------------------------------------------------------------
    # Aktionsbuttons (NORMAL START | TIMELAPSE START | RTH)
    # -------------------------------------------------------------------------
    def _build_action_buttons(self):
        ...
        # Preset-Buttons in zweiter Zeile
        preset_row = ctk.CTkFrame(self)
        preset_row.pack(fill="x", padx=12, pady=(0, 4))

        ctk.CTkButton(preset_row, text="Preset laden", width=150,
                      command=self._load_preset).pack(side="left", padx=(8, 4), pady=4)
        ctk.CTkButton(preset_row, text="Preset speichern", width=150,
    ...
    def _send_normal(self):
        ...
        if error:
            messagebox.showwarning("Ungültige Eingabe", error)
            return
    ...
    def _send(self, cmd: str):
        try:
            ...
            self._log(f"Gesendet: {cmd}")
        except ConnectionError as e:
            messagebox.showerror("Sendefehler", str(e))

    # -------------------------------------------------------------------------
    # Preset Laden / Speichern
    # -------------------------------------------------------------------------
    def _save_preset(self):
        """Liest aktuelle Werte aus beiden Tabs und speichert sie als .json."""
        ...
        if path:
            ...
            self._log(f"Preset gespeichert: {path}")

    def _load_preset(self):
        """Lädt eine .json-Datei und befüllt beide Tabs."""
        ...
        try:
            ...
            self._log(f"Preset geladen: {data.get('name', path)}")
        except (KeyError, ValueError) as e:
            messagebox.showerror("Preset-Fehler", f"Ungültiges Preset-Format:\n{e}")
    ...
    def _log(self, text: str):
        """Fügt eine Zeile in das Log-Feld ein (thread-safe via after)."""
    ...
    def _log_receive(self, text: str):
        """Callback für eingehende Controller-Antworten (aus Hintergrundthread)."""
    ...
    def _check_connected(self) -> bool:
        if not self.serial.is_connected():
            messagebox.showwarning("Nicht verbunden", "Bitte zuerst einen Port verbinden.")
            return False
        return True
